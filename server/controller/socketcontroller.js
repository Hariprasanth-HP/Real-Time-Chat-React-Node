...
module.exports.initializeUser = async (socket) => {
  ...
  const messageList = await redisClient.lrange(
    `messages:${socket.user.username}`,
    0,
    -1
  );
...
module.exports.addFriend = async (socket, friendName, cb) => {
  ...
  console.log("friendName === socket.user.username", friendName, friend);
...
module.exports.addMessages = async (socket, friendName, cb) => {
  if (friendName === socket.user.username) {
    cb({ done: false, errorMsg: "Cannot add self!" });
    return;
  }
